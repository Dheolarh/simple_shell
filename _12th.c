#include "shell.h"

/**
 * get_history_file - retrieves the history file path
 * @info: contains the program's info
 * Return: an allocated string containing the history file path
 */

char *get_history_file(info_t *info)
{
	/* Retrieve the HOME environment variable */
	char *dir = _getenv(info, "CD=");

	if (!dir)
		return (NULL);

	/* Allocate memory for the history file path */
	char *buf = malloc(sizeof(char) * (_strlen(dir) + _strlen(HIST_FILE) + 2));

	if (!buf)
		return (NULL);

	/* Concatenate the HOME directory and the history file name */
	buf[0] = 0;
	_strcpy(buf, dir);
	_strcat(buf, "/");
	_strcat(buf, HIST_FILE);

	return (buf);
}

/**
 * write_history - writes the command history to a file
 * @info: contains the program's info
 * Return: 1 on success, else -
 */
int write_history(info_t *info)
{
	ssize_t fd;
	char *filename = get_history_file(info);
	list_t *node = NULL;

	if (!filename)
		return (-1);

	/* Open the history file for writing */
	fd = open(filename, O_CREAT | O_TRUNC | O_RDWR, 0644);
	free(filename);
	if (fd == -1)
		return (-1);

	/* Write each command in the history list to the file */
	for (node = info->history; node; node = node->next)
	{
		_putsfd(node->str, fd);
		_putfd('\n', fd);
	}
	_putfd(BUF_FLUSH, fd);
	close(fd);

	return (1);
}

/**
 * read_history - reads the command history from a file
 * @info: contains the program's info
 * Return: the number of commands read from the history file
 */
{
	int i, last = 0, linecount = 0;
	ssize_t fd, rdlen, fsize = 0;
	struct stat st;
	char *buf = NULL, *filename = get_history_file(info);

	if (!filename)
		return (0);

	fd = open(filename, O_RDONLY);
	free(filename);
	if (fd == -1)
		return (0);
	if (!fstat(fd, &st))
		fsize = st.st_size;
	if (fsize < 2)
		return (0);
	buf = malloc(sizeof(char) * (fsize + 1));
	if (!buf)
		return (0);
	rdlen = read(fd, buf, fsize);
	buf[fsize] = 0;
	if (rdlen <= 0)
		return (free(buf), 0);
	close(fd);
	for (i = 0; i < fsize; i++)
		if (buf[i] == '\n')
		{
			buf[i] = 0;
			build_history_list(info, buf + last, linecount++);
			last = i + 1;
		}
	if (last != i)
		build_history_list(info, buf + last, linecount++);
	free(buf);
	info->histcount = linecount;
	while (info->histcount-- >= HIST_MAX)
		delete_node_at_index(&(info->history), 0);
	renumber_history(info);
	return (info->histcount);
}

/**
 * build_history_list - adds an entry to the command history list
 * @info: the parameter struct
 * @buf: the command to add to the history list
 * @linecount: the line number of the command in the history file
 *
 * Return: Always 0
 */
int build_history_list(info_t *info, char *buf, int linecount)
{
	list_t *node = NULL;

	if (info->history)
		node = info->history;
	add_node_end(&node, buf, linecount);
	/* Add the command to the end of the history list */
	if (!info->history)
		info->history = node;
	return (0);
}

/**
 * renumber_history - renumbers the command history list after changes
 * @info: the parameter struct
 *
 * Return: the new number of commands in the history list
 */
int renumber_history(info_t *info)
{
	list_t *node = info->history;
	int i = 0;

	while (node)
	{
		node->num = i++;
		node = node->next;
	}
	return (info->histcount = i);
}
